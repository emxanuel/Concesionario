...
const EditModal: React.FC<Props> = ({ vehicle, client, show }) => {
    ...
    return (
        <div id='editModal' className={`w-full -mt-16 h-full bg-black bg-opacity-70 absolute left-0 justify-center items-center ${show ? 'flex' : 'hidden'}`}>
            <div><button onClick={() => {
                const element = document.getElementById('editModal') as HTMLElement
                toggleShowElement(element)
            }
            }><i className='fa-solid fa-x absolute top-0 right-0 p-10 text-white text-3xl' /></button></div>
